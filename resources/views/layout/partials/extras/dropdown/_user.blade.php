@if (config('layout', 'extras/user/dropdown/style') == 'light')
    ...
    <div class="d-flex align-items-center p-8 rounded-top">
            {{-- Text --}}
            <div class="text-white m-0 flex-grow-1 mr-3 font-size-h5 font-weight-bold text-capitalize">{{ $currentUser->name}}</div>
            <div class="label label-lg font-weight-bold label-inline text-capitalize">{{ $currentUser->roles->name }}</div>
    </div>
